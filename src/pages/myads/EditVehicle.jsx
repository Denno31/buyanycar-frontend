...
import { UPDATE_ADD } from "../../mutations/vehicleMutations";
...
const CreateAd = () => {
  ...
  React.useEffect(() => {
    if (!loading && data) {
      ...
      setColor(data?.getVehicle?.color);
      setCondition(data?.getVehicle?.condition);
      setTransmission(data?.getVehicle?.transmission);
      setBodyType(data?.getVehicle?.bodyType);
      setFuelType(data?.getVehicle?.fuel);
      setRegistered(data?.getVehicle?.registered);
      setEngineSize(data?.getVehicle?.engineSize);
      setMileage(data?.getVehicle?.mileage);
      setPrice(data?.getVehicle?.price);
      setPhoneNumber(data?.getVehicle?.phoneNumber);
      setDescription(data?.getVehicle?.description);
    }
  }, [data, loading]);
  ...
  const [editVehicle, { loading: loadingEditVehicle }] = useMutation(
    UPDATE_ADD,
    {
      update(proxy, result) {
        // console.log(result);
